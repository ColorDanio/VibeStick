...
class SessionStore:
        ...
        for tool_id, sessions in found.items():
            ...
            for s in sessions:
                ...
                status = protocol.SessionStatus(
                    ...
                    quota_pct=float(getattr(s, "quota_pct", -1.0)),
                    tokens=int(getattr(s, "tokens", -1)),
    ...
    def _presence_record(self, tool_id: str | None) -> SessionRecord | None:
        ...
        status = protocol.SessionStatus(
            ...
            quota_pct=linked_status.quota_pct if linked_status is not None else -1.0,
            tokens=linked_status.tokens if linked_status is not None else -1,
